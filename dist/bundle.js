...
var Utils = {
	pointsAngle: function pointsAngle(p1, p2) {
		return Math.atan2(p2.y - p1.y, p2.x - p1.x);
	},
	degreesToRadians: function degreesToRadians(a) {
	...
	vectorLength: function vectorLength(p1, p2) {
		return Math.sqrt((p2.x - p1.x) ** 2 + (p2.y - p1.y) ** 2);
	},
	isPointInsideRect: function isPointInsideRect(p, rect) {
		if (rect.x1 <= p.x && rect.x2 >= p.x && rect.y1 <= p.y && rect.y2 >= p.y) return true;
		return false;
	}
};
...
var Vector2 = function Vector2(x, y) {
	function add(prop, val) {
		if (Number.isFinite(val)) this[prop] += val;
	}
...
var Bullet = function Bullet(screen_ctx, parent, x, y, angle) {
    var canvas = document.createElement("canvas");
    var ctx = canvas.getContext('2d');

    var spread = 0.01;
    angle = _utils2.default.random(angle * (1 - spread), angle * (1 + spread));
    var speed = 2000;
    var x_speed = speed * Math.cos(angle);
    var y_speed = speed * Math.sin(angle);
    var max_age = 0.5;
    var age = 0;
    var damage = 3;
    ...
    function draw_buffer() {
        ...
        var s = "#" + Math.round((1 - powerPercentage()) * 255).toString(16).pad('0', 2).repeat(3);
        ctx.fillStyle = s;
        ctx.fillRect(-6, -1, 12, 2);
    }
    ...
    function move(dt, tracers) {
        var target_delta = 0.001;
        var repeat = Math.round(dt / target_delta);
        var step = dt / repeat;
        while (repeat--) {
            do_move(step);
            ...
            try {
                for (var _iterator = tracers[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                    var tracer = _step.value;

                    if (tracer !== parent) {
                        if (tracer.checkCollision({ x: x, y: y })) {
                            destroy();
                            return tracer;
                        }
                    }
...
var game = (0, _game2.default)(canvas);
...
var _tracer = __webpack_require__(25);
...
var Game = function Game(canvas) {
	...
	var keys_not_released = new Set();
	var bullets = [];
	var tracers = [];
	var client = (0, _client2.default)('http://127.0.0.1:16217');
	...
	client.bind('tracer_disconnect', function (id) {
		tracers = tracers.filter(function (tr) {
			return tr.getId() !== id;
		});
	...
	client.bind('tracer_update', function (data) {
		var tracer = tracers.find(function (tr) {
			return tr.getId() === data.id;
		});
		if (tracer) {
			tracer.setData(data);
		}
	...
	function createTracer(data) {
		tracers.push((0, _tracer2.default)(canvas_ctx, game_api, data.x, data.y, parseFloat(data.angle), data.id));
	}
	...
	window.onkeyup = function (e) {
		return onKeyUp(e.key);
	};
	canvas.onmousedown = function (e) {
		return onKeyDown(getKey(e));
	};
	canvas.onmouseup = function (e) {
		return onKeyUp(getKey(e));
	};
	...
	function draw() {
		...
		tracers.forEach(function (tracer_player) {
			return tracer_player.draw();
		});
	...
	var prev_timestamp = void 0;
	function getDiff(timestamp) {
		var diff = 0;
		if (prev_timestamp) diff = (timestamp - prev_timestamp) / 1000;
		prev_timestamp = timestamp;
		return diff;
	}

	function controls(diff) {
		var move_dir = (0, _utils.Vector2)(0, 0);
		var move_map = [['w', (0, _utils.Vector2)(1, 0)], ['s', (0, _utils.Vector2)(-1, 0)], ['d', (0, _utils.Vector2)(0, 1)], ['a', (0, _utils.Vector2)(0, -1)]];
		move_map.forEach(function (pair) {
			var _pair = _slicedToArray(pair, 2),
			    key = _pair[0],
			    dir = _pair[1];

			if (keys_pressed.has(key)) move_dir.addNumbers(dir);
		});
		if (move_dir.x || move_dir.y) {
			var angle = _utils.Utils.pointsAngle({ x: 0, y: 0 }, move_dir);
			tracer_player.move(angle, diff);

			['shift', 'rmb'].forEach(function (key) {
		...
		if (keys_pressed.has('q') && !keys_not_released.has('q')) {
			...
			var rect = tracer_player.checkCollision({ x: 200, y: 200 });
			//console.log(Utils.radiansToDegrees(tracer_player.getAngle()), rect, tracer_player);
		}

		if (keys_pressed.has('lmb')) tracer_player.shoot();
	}
	...
	function processBullets(dt) {
		bullets.forEach(function (bullet) {
			var hit_tracer = bullet.move(dt, tracers);
			if (hit_tracer) hit_tracer.hit(bullet.getDamage());
		});
	...
	function updateTracers(dt) {
		...
		tracers.forEach(function (tracer_player) {
			return tracer_player.update(dt);
		});
	}

	function step(timestamp) {
		var dt = getDiff(timestamp);
	...
	var tracer_player = (0, _tracer2.default)(canvas_ctx, game_api, _utils.Utils.random(100, 800), _utils.Utils.random(100, 500), 0);
	client.sendData('tracer_create', tracer_player.getData());
	var tracer_comp = (0, _tracer2.default)(canvas_ctx, game_api, 800, 300, 0);
...
var Tracer = function Tracer(screen_ctx, game_api, x, y) {
	var init_angle = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 0;
	var id = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : false;
	...
	var max_ammo = 30;
	var ammo = max_ammo;
	var between_shots_time = 0.05;
	var current_shot_delay = 0;
	var reload_time = 2;
	...
	function update_delays(dt) {
		...
		var reload_time_left_prev = reload_time_left;
		reload_time_left = Math.max(0, reload_time_left - dt);
		if (reload_time_left === 0 && reload_time_left_prev > 0) ammo = max_ammo;
	}
	...
	function move(add_angle, dt) {
		var x_speed = speed * dt * Math.cos(angle + add_angle);
		var y_speed = speed * dt * Math.sin(angle + add_angle);
		x -= x_speed;
		y -= y_speed;
	...
	function shoot() {
		if (current_shot_delay <= 0 && ammo > 0) {
			game_api.addBullet((0, _bullet2.default)(screen_ctx, this, x, y, angle));
	...
	function checkCollision(pos) {
		var point_angle = _utils2.default.pointsAngle({ x: x, y: y }, pos);
		var r = _utils2.default.vectorLength({ x: x, y: y }, pos);
		var rotated_pos = { x: x + r * Math.cos(point_angle + angle), y: y + r * Math.sin(point_angle + angle) };
		return _utils2.default.isPointInsideRect(rotated_pos, getRect());
	}
	...
	return {
		draw: draw,
		rotate: rotate,
		rotateToMouse: function rotateToMouse(pos) {
			return rotate(_utils2.default.pointsAngle(pos, { x: x, y: y }));
		},
		move: move,
		blink: blink,
		reload: reload,
		shoot: shoot,
		update: update,
		getAmmo: function getAmmo() {
			return ammo;
		},
		getReloadProgress: function getReloadProgress() {
			return (reload_time - reload_time_left) / reload_time;
		},
...
var Client = function Client(address) {

	var socket = _socket2.default.connect(address, function (socket) {
	...
	return {
		sendData: function sendData(event, data) {
			if (!data.isEmpty()) socket.emit(event, data);
		},
